...
| 工作 | 需要回答的问题 |
|---|---|
| 模型分析 | 模型有哪些特殊结构，哪些语义不能被后端优化改变 |
| 设备适配 | 不同 SOC 支持哪些 Shape 和 Kernel，Fallback 应放在哪里 |
| 图模式 | Capture、Update、Replay 的 Backend、参数和状态是否一致 |
| 量化/MoE/MTP | 优化路径是否保持权重、激活、Cache 和通信语义 |
| 精度性能 | 问题怎样定位，收益怎样在固定 Workload 下证明 |
| 文档与 CI | 当前结果、证据、边界和长期看护是否完整 |
...
对外发布的适配文档应先保证结构清楚，再使用自然、易懂的表达。每篇文档按以下顺序组织：

```text
做了什么：本篇覆盖哪些适配工作
为什么做：模型结构、设备能力或用户场景提出了什么要求
遇到的问题：具体设备、模型、模式、报错或精度现象
如何分析：调用链、控制组、A/B 实验和证据
解决方案：实现原理、代码边界和方案取舍
结果与收益：正确性、性能和维护性改善
当前边界：已确认结论与待验证项
```

总览保持系统性，提供文档链接、适配范围、工作内容、主要问题、当前结果和边界。详细文档再展开错误、分析与设计。先用通俗语言解释原理，再给公式、参数和代码路径；语气可以随和，但避免随意化标题和过度故事化表达。
