...
这一篇完成 Gemma4 四个模型变体的结构拆解，并把模型结构转换成后端可以使用的设计约束。

| 分析内容 | 目的 | 对后端的直接影响 |
|---|---|---|
| Sliding/Global 交错层 | 明确不同 layer 的 Attention 语义 | Head size、mask、workspace 和 backend 不能混用 |
| GQA 与 TP 后 shape | 明确本地 Q/K/V 布局 | FIA、PA 和 KV Cache 参数必须使用正确 head 数 |
| `attention_k_eq_v` | 确认 Global layer 的权重共享 | Loader 和量化路径允许合法缺少独立 V |
| Dense/MoE FFN | 确认激活、Router 和专家结构 | GELU-TANH、Top-8 和通信路径保持一致 |
| PLE、YOCO、KV Sharing | 明确逐层输入和 Cache 所有权 | Target layer 读取 producer KV，但不能覆盖它 |
| Multimodal | 明确多模态 token 如何进入 Decoder | Prefill shape、Graph bucket 和性能验证包含图片 |

这一阶段的产出不是某个设备分支，而是一组后续 A2/A3/A5、Graph 和量化实现都必须遵守的模型语义。
...
1. **配置证据**：Head、Layer Pattern、Window、Expert、Activation 等静态信息。
2. **代码证据**：Module 构造、Forward 数据流、Weight Loader 和 Cache 写入条件。
3. **Shape 证据**：Prefill/Decode、TP 前后和 KV Cache 中的真实 tensor shape。
4. **对照证据**：Dense/MoE、Sliding/Global、Producer/Target、BF16/W8A8 的差异。
...
## 7. 分析结果：Q/K/V Normalization 与 Scaling
...
## 11. 分析结果：E2B/E4B 的 YOCO 与 KV Sharing

### 11.1 Self-Decoder / Cross-Decoder
...
模型分析最终要落到代码边界，而不是停留在架构图：

| 模型结论 | 适配设计 |
|---|---|
| 256/512 head 交错 | Backend 选择接收 head size；Workspace 覆盖 bucket 内最大需求 |
| A2/A3 FIA 不支持 512 head | 只对 Global layer 设计 PA/large-head fallback |
| Global K=V | Weight loader 将 K 映射到 K/V slot；非 K=V 仍严格检查 V |
| MoE 使用 GELU-TANH | 浮点和量化 expert 读取模型 activation，不默认 SwiGLU |
| Router Top-8 动态变化 | Graph Replay 更新 active token、dispatch 和 combine metadata |
| KV Sharing | 显式保存 producer、metadata owner 和当前执行 layer |
| 多模态扩展 Prefill | Graph 与性能测试加入文本和图片 workload |
...
通过结构分析，我们已经能为每个模型变体明确 Attention、FFN、Cache 和量化差异。后续实现是否正确，需要使用下表逐项验证：
